...
const Currency = (props) => {
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_URL}/api/getallcurrencies`)
      ...
      .then((res) => {
        setAllCurrensies(res.AllCurrensies);
        dispatch(setCurrenciesAction(res.AllCurrensies));

        const currs1 = res.AllCurrensies?.filter((item) => item.id < 14);
        const currs2 = res.AllCurrensies?.filter(
          (item) => item.id >= 14 && item.id <= 18
        );
        setCurrency(currs1);
        setCurrency2(currs2);
      })
