...
const Main = () => {
  ...
  const currenciesFromStore = useSelector(
    (state) => state.Reducer.currencies
  );
  ...
  const updateCurrencyHandler = async () => {
    ...
    try {
      _response = await fetch(
        `${process.env.REACT_APP_URL}/api/updateCurrency`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div style={{ webkitAppRegion: "drag" }}>
      <div
        style={{ direction: "rtl" }}
        className="grid  grid-flow-col-2 h-[490px]  -mt-4   "
      >
        <div>
          <Currency />
        </div>
        <div className="w-[50%] relative z-10 right-[48%] -top-[100%]">
          <Coin />
        </div>
      </div>
      ...
      <div
        ...
        className="flex flex-row-reverse  w-[80%] mx-auto justify-around "
      >
        <button
